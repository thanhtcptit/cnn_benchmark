...
class ConvBlock(tf.keras.Model):
    ...
    def __init__(self, num_filters, data_format, bottleneck, weight_decay=1e-4,
        ...
        self.conv2 = tf.keras.layers.Conv2D(
            num_filters, (3, 3), padding="same", use_bias=False,
        ...
        if self.bottleneck:
            self.conv1 = tf.keras.layers.Conv2D(
                inter_filter, (1, 1), padding="same", use_bias=False,
                data_format=data_format, kernel_initializer="he_normal",
                kernel_regularizer=l2(weight_decay))
...
class TransitionBlock(tf.keras.Model):
    ...
    def __init__(self, num_filters, data_format, weight_decay=1e-4,
        ...
        self.conv = tf.keras.layers.Conv2D(
            num_filters, (1, 1), padding="same", use_bias=False,
            data_format=data_format, kernel_initializer="he_normal",
            kernel_regularizer=l2(weight_decay))
...
class DenseNet(tf.keras.Model):
    ...
    Arguments:
        ...
        compression: reducing the number of inputs(filters) to the
            transition block.
        weight_decay: weight decay
        rate: dropout rate.
        pool_initial: If True add a 7x7 conv with stride 2 followed by
            3x3 maxpool, else do a 3x3 conv with stride 1.
    ...
    def __init__(self, depth_of_model, growth_rate, num_of_blocks,
        ...
        if isinstance(self.num_layers_in_each_block, list) or isinstance(
            ...
            if self.num_layers_in_each_block == -1:
                if self.num_of_blocks != 3:
                    raise ValueError(
                        "Number of blocks must be 3 " +
                        "if num_layers_in_each_block is -1")
        ...
        self.conv1 = tf.keras.layers.Conv2D(
            self.num_filters, init_filters, strides=stride,
        ...
        for i in range(self.num_of_blocks):
            self.dense_blocks.append(DenseBlock(
                self.num_layers_in_each_block[i], self.growth_rate,
                self.data_format, self.bottleneck, self.weight_decay,
                self.dropout_rate))
            if i + 1 < self.num_of_blocks:
                self.transition_blocks.append(TransitionBlock(
                    num_filters_after_each_block[i + 1], self.data_format,
...
def inference(images, keep_probability, phase_train=True,
    ...
    net = DenseNet(depth_of_model=22, growth_rate=12, num_of_blocks=3,
                   ...
                   data_format="channels_last", dropout_rate=keep_probability)
    return net(images, training=phase_train), None
